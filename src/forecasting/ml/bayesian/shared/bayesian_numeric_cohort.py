...
def resolve_bayesian_cohort_assets(
    *,
    parquet_root: Path,
    ...
    if not interval_values:
        raise RuntimeError("No intervals resolved for Bayesian cohort selection.")
    ...
    if not common_assets:
        raise RuntimeError("No shared eligible assets were found across the requested Bayesian intervals.")
    selected_assets, _alias_map = select_representative_assets(
        common_assets,
        seed=int(seed),
        asset_count=int(asset_count),
        required_symbols=FIXED_BAYESIAN_NUMERIC_COHORT,
    )
