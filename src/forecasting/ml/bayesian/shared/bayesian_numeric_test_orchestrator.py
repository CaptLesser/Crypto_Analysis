...
MODEL_ORDER = list(BAYESIAN_NUMERIC_BRANCHES)


@dataclass(frozen=True)
class BayesianNumericTestModuleSpec:
    model_key: str
    display_name: str
    stage1_module: str
    stage2_module: str
...
MODEL_SPECS: Dict[str, BayesianNumericTestModuleSpec] = {
    "dlm_tvp": BayesianNumericTestModuleSpec(
        model_key="dlm_tvp",
        display_name="DLM TVP",
        stage1_module=BAYESIAN_STAGE1_ENTRYPOINTS["dlm_tvp"],
        stage2_module=BAYESIAN_STAGE2_ENTRYPOINTS["dlm_tvp"],
        stage3_module=BAYESIAN_STAGE3_ENTRYPOINTS["dlm_tvp"],
    ),
    "stochastic_vol": BayesianNumericTestModuleSpec(
        model_key="stochastic_vol",
        display_name="Stochastic Vol",
        stage1_module=BAYESIAN_STAGE1_ENTRYPOINTS["stochastic_vol"],
        stage2_module=BAYESIAN_STAGE2_ENTRYPOINTS["stochastic_vol"],
        stage3_module=BAYESIAN_STAGE3_ENTRYPOINTS["stochastic_vol"],
    ),
    "dynamic_regression_shrinkage": BayesianNumericTestModuleSpec(
        model_key="dynamic_regression_shrinkage",
        display_name="Dynamic Regression Shrinkage",
        stage1_module=BAYESIAN_STAGE1_ENTRYPOINTS["dynamic_regression_shrinkage"],
        stage2_module=BAYESIAN_STAGE2_ENTRYPOINTS["dynamic_regression_shrinkage"],
        stage3_module=BAYESIAN_STAGE3_ENTRYPOINTS["dynamic_regression_shrinkage"],
    ),
    "copula_dependency": BayesianNumericTestModuleSpec(
        model_key="copula_dependency",
        display_name="Copula Dependency",
        stage1_module=BAYESIAN_STAGE1_ENTRYPOINTS["copula_dependency"],
        stage2_module=BAYESIAN_STAGE2_ENTRYPOINTS["copula_dependency"],
        stage3_module=BAYESIAN_STAGE3_ENTRYPOINTS["copula_dependency"],
    ),
    "tail_risk": BayesianNumericTestModuleSpec(
        model_key="tail_risk",
        display_name="Tail Risk",
        stage1_module=BAYESIAN_STAGE1_ENTRYPOINTS["tail_risk"],
        stage2_module=BAYESIAN_STAGE2_ENTRYPOINTS["tail_risk"],
        stage3_module=BAYESIAN_STAGE3_ENTRYPOINTS["tail_risk"],
    ),
}

RUN_STATE_FILE = "bayesian_numeric_test_orchestrator_state.json"
RUN_SUMMARY_FILE = "bayesian_numeric_stage3_artifacts.json"
DEFAULT_OUTPUT_DIR = Path("logs") / "diagnostics" / "bayesian_numeric_family_test_orchestrator"
STAGE0_PROFILE_JSON = "bayesian_numeric_stage0_profile.json"
STAGE0_CANDIDATES_CSV = "bayesian_numeric_stage0_candidates.csv"
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Sequential Stage-1/2/3 test orchestrator for the Bayesian numerics family.")
...
def selected_stage0_asset_count(run_root: Path) -> Optional[int]:
    assets = selected_stage0_assets(run_root)
    return len(assets) if assets else None


def selected_stage0_asset_workers(run_root: Path) -> Optional[int]:
    raw = selected_stage0_profile(run_root).get("asset_workers")
    if raw is None:
        return None
    try:
...
def selected_stage0_model_threads(run_root: Path) -> Optional[int]:
    raw = selected_stage0_profile(run_root).get("model_threads")
    if raw is None:
        return None
    try:
        return max(1, int(raw))
    ...
    *,
    ...
    payload: Dict[str, Any] = {
        ...
        "env_hints": {
            ...
            "BAYESIAN_NUMERIC_MODEL_THREADS": env.get("BAYESIAN_NUMERIC_MODEL_THREADS"),
        },
    }
...
def build_stage0_command(args: argparse.Namespace, run_root: Path) -> list[str]:
    command = [sys.executable, "-m", BAYESIAN_STAGE0_ENTRYPOINT]
...
def build_stage1_command(spec: BayesianNumericTestModuleSpec, args: argparse.Namespace, paths: ModelPaths, *, run_root: Path) -> list[str]:
    ...
    stage0_assets_csv = ",".join(selected_stage0_assets(run_root))
    command_flag(command, "--assets", args.assets if str(args.assets).strip() else stage0_assets_csv)
    command_flag(command, "--asset-count", args.stage1_asset_count if args.stage1_asset_count is not None else selected_stage0_asset_count(run_root))
    command_flag(command, "--train-window-months", args.stage1_train_window_months)
    return command


def build_stage2_command(spec: BayesianNumericTestModuleSpec, args: argparse.Namespace, paths: ModelPaths, *, run_root: Path) -> list[str]:
    ...
    command_flag(command, "--asset-workers", selected_stage0_asset_workers(run_root))
    command_flag(command, "--model-threads", selected_stage0_model_threads(run_root))
    return command


def build_stage3_command(spec: BayesianNumericTestModuleSpec, args: argparse.Namespace, paths: ModelPaths, *, stage2_manifest: Path, survivor_json: Path, run_root: Path) -> list[str]:
    study_prefix = str(args.stage3_study_name_prefix).strip() or f"bayesian_numeric_family_{run_root.name}_{spec.model_key}"
    ...
    if selected_threads is None:
        selected_threads = selected_stage0_model_threads(run_root)
...
def resolve_stage3_storage_path(spec: BayesianNumericTestModuleSpec, args: argparse.Namespace, paths: ModelPaths) -> Path:
    ...
    assert_write_allowed(storage_path, "bayesian test branch optuna storage", roots=resolve_sandbox_output_roots(args))
    return storage_path
...
def collect_stage0_status(run_root: Path) -> Dict[str, Any]:
    ...
    return {
        "module": BAYESIAN_STAGE0_ENTRYPOINT,
...
def run_stage1(spec: BayesianNumericTestModuleSpec, args: argparse.Namespace, run_root: Path, env: Dict[str, str]) -> None:
    ...
    command = build_stage1_command(spec, args, paths, run_root=run_root)
...
def run_stage2(spec: BayesianNumericTestModuleSpec, args: argparse.Namespace, run_root: Path, env: Dict[str, str]) -> Path:
    ...
    command = build_stage2_command(spec, args, paths, run_root=run_root)
...
def run_stage3(spec: BayesianNumericTestModuleSpec, args: argparse.Namespace, run_root: Path, env: Dict[str, str], *, stage2_manifest: Path) -> None:
    paths = model_paths(run_root, spec.model_key)
...
def run_orchestrator(args: argparse.Namespace) -> Path:
    ...
    assert_test_branch_sandbox_launch(args, run_root, env, family_key="bayesian")
    if args.resume_run:
