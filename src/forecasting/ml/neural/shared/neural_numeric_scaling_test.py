...
from src.forecasting.common.forecast_family_core import discover_edge_and_min, read_feature_window_columns, write_json_atomic
...
from src.forecasting.ml.neural.shared.neural_diagnostic_analysis import analyze_manifest_for_model
...
DEFAULT_PROGRESS_LOG_SECONDS = 30.0
NO_WORK_QUALITY_STATUSES = {"ineligible_insufficient_sequence_history"}
...
def _summary_complete(summary: Optional[Dict[str, Any]]) -> bool:
    ...
    if isinstance(by_asset, dict) and bool(by_asset):
        return True
    quality = summary.get("quality") if isinstance(summary.get("quality"), dict) else {}
    quality_status = str(summary.get("quality_status") or quality.get("status") or "")
    return isinstance(by_asset, dict) and quality_status in NO_WORK_QUALITY_STATUSES
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Neural numeric Stage 2 diagnostic scaling test")
...
def _load_module(model_key: str):
    return __import__(f"src.forecasting.ml.neural.{model_key}.numerics", fromlist=["MODULE_SPEC"])
...
def _origin_metrics(
    ...
    *,
    ...
    y_vec = pd.to_numeric(label_values, errors="coerce").to_numpy(dtype=float)
    candidate_cols = [str(col) for col in getattr(module.MODULE_SPEC, "dynamic_feature_candidates", ()) if str(col) in merged.columns]
    ...
    train_bars = _training_window_bars(int(interval), int(training_window_months))
    effective_history_bars = min(int(train_bars), max(64, int(seq_len)))
    preds: List[float] = []
    actuals: List[float] = []
    pred_ts: List[int] = []
    ...
    if progress_label:
        progress_line(
            ...
            f"train_bars={int(train_bars)} effective_history_bars={int(effective_history_bars)} seq_len={int(seq_len)}"
        )
    for idx in range(len(ts_vec)):
        ...
        now = time.monotonic()
        if progress_label and (now - last_progress_at) >= float(DEFAULT_PROGRESS_LOG_SECONDS):
            progress_line(
                f"{progress_label}: progress idx={int(idx)}/{int(len(ts_vec) - 1)} "
                f"predictions={int(len(preds))} elapsed_s={int(now - started_at)}"
            )
...
def _seq_len_for_interval(module: Any, interval: int) -> int:
    runtime_params = dict(getattr(module.MODULE_SPEC, "runtime_params", {}) or {})
...
def _training_window_bars(interval: int, training_window_months: int) -> int:
    return max(64, int(training_window_months) * 30 * 24 * 60 // max(1, int(interval)))


def _min_required_history_bars(module: Any, seq_len: int) -> int:
    model_params = dict(getattr(module.MODULE_SPEC, "model_params", {}) or {})
    seq_floor = int(model_params.get("seq_len_floor", 0) or 0)
    effective_seq_len = max(int(seq_len), int(seq_floor))
    model_id = str(getattr(module.MODULE_SPEC, "model_id", ""))
...
def _sequence_history_profile(module: Any, *, interval: int, training_window_months: int) -> Dict[str, Any]:
    seq_len = _seq_len_for_interval(module, int(interval))
    train_bars = _training_window_bars(int(interval), int(training_window_months))
    min_required = _min_required_history_bars(module, int(seq_len))
    status = "eligible_sequence_history"
    reason = "training_window_bars_meet_min_required_history_bars"
    if int(train_bars) < int(min_required):
        status = "ineligible_insufficient_sequence_history"
        reason = "training_window_bars_below_min_required_history_bars"
    return {
        "status": status,
        "reason": reason,
        "interval_minutes": int(interval),
        "training_window_months": int(training_window_months),
        "training_window_bars": int(train_bars),
        "seq_len": int(seq_len),
        "min_required_history_bars": int(min_required),
    }
...
def _score_combo_shard(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    progress_line(
        ...
        f"assets={int(len(assets))} combos={int(len(payload.get('task_horizons', ()) or ())) }"
    )
    ...
    for task, horizon_minutes in payload.get("task_horizons", ()):
        ...
        combo_profile = (
            resolve_execution_profile(
                ...
                task=str(task),
                dynamic_feature_candidates=getattr(module.MODULE_SPEC, "dynamic_feature_candidates", ()),
                needs_dynamic_features=bool(getattr(module.MODULE_SPEC, "needs_dynamic_features", False)),
            )
            if feature_profile_json is not None and feature_profile_json.exists()
            else None
        )
        ...
        frames = grouped_frame_cache.get(cache_key)
        if frames is None:
            frames = {}
            for asset in assets:
                base_ohlcvt_frame = base_ohlcvt_cache.get(str(asset))
                if base_ohlcvt_frame is None:
                    base_ohlcvt_frame = read_ohlcvt(
                        asset=str(asset),
                        interval_min=int(interval),
                    ...
                    base_ohlcvt_cache[str(asset)] = base_ohlcvt_frame
        ...
        for asset, frame in frames.items():
            ...
            progress_line(
                f"Stage 2 worker {combo_label} asset={str(asset)}: complete forecasts={int(len(preds))} "
                f"rmse={float(rmse):.6f} mae={float(mae):.6f}"
            )
        progress_line(f"Stage 2 worker {combo_label}: combo-done")
    progress_line(
...
def run_stage_for_model(model_key: str) -> Path:
    ...
    if args.model_threads is not None:
        ...
        os.environ["NEURAL_NUMERIC_MODEL_THREADS"] = str(max(1, int(args.model_threads)))
    configure_neural_thread_env()
    if str(model_key) not in NEURAL_NUMERIC_BRANCHES:
        raise RuntimeError(f"unsupported Neural model key: {model_key}")
    ...
    if bool(args.staged) and feature_profile_json is None:
        raise SystemExit("Staged Neural Stage-2 run blocked: missing --feature-profile-json.")
    ...
    for interval in sorted(set(int(c[0]) for c in combos)):
        ...
        if not interval_pairs:
            continue
        progress_line(
            f"Neural Stage 2 {str(model_key)}: interval-start interval={int(interval)}m combos={int(len(interval_pairs))}"
        )
        interval_edges = []
        ...
        for training_window_months in train_windows:
            ...
            if _summary_complete(summary):
                progress_line(
                    f"Neural Stage 2 {str(model_key)}: window-skip interval={int(interval)}m window={int(training_window_months)}m reason=complete"
                )
            ...
            progress_line(
                f"Neural Stage 2 {str(model_key)}: window-start interval={int(interval)}m window={int(training_window_months)}m "
                f"run_dir={str(run_dir)} history_start_ts={int(history_start_ts)} common_edge={int(common_edge)}"
            )
            ...
            sequence_history = _sequence_history_profile(
                module,
                interval=int(interval),
                training_window_months=int(training_window_months),
            )
            if str(sequence_history["status"]) in NO_WORK_QUALITY_STATUSES:
                quality = {
                    "status": str(sequence_history["status"]),
                    "reason": str(sequence_history["reason"]),
                    "requested_assets": int(len(assets)),
                    "assets_with_metrics": 0,
                    "completed_job_shards": 0,
                    "completed_combo_jobs": 0,
                    "sequence_history": dict(sequence_history),
                }
                summary = {
                    ...
                    "config": {
                        ...
                        "worker_mode": "ineligible_no_work_preflight",
                        "job_shard_count": 0,
                        "completed_job_shards": 0,
                        "completed_combo_jobs": 0,
                        "grouped_load_cache_entries": 0,
                        "history_start_ts": int(history_start_ts),
                        "exact_history_required": True,
                        "seed_ts": int(common_edge - int(args.forecast_days) * 86400),
                        "accuracy_end_ts": int(common_edge),
                        "forecast_target_month_start_utc": datetime.fromtimestamp(int(common_edge), tz=timezone.utc).replace(day=1, hour=0, minute=0, second=0, microsecond=0).isoformat(),
                        "sequence_history": dict(sequence_history),
                    },
                ...
                progress_line(
                    ...
                    f"min_required_history_bars={int(sequence_history['min_required_history_bars'])} run_summary={str(run_summary)}"
                )
            ...
            progress_line(
                f"Neural Stage 2 {str(model_key)}: window-dispatch interval={int(interval)}m window={int(training_window_months)}m "
                f"worker_budget={int(worker_budget)} shard_count={int(len(combo_shards))}"
            )
                ...
                shard_payloads = [
                    {
                        ...
                        "shard_label": f"{str(model_key)}-i{int(interval)}-w{int(training_window_months)}-s{int(index + 1)}of{int(len(combo_shards))}",
                    ...
                    _score_combo_shard(
                        {
                            ...
                            "assets": list(assets),
                            "task_horizons": [(str(task), int(horizon)) for task, horizon in shard],
                            "shard_label": f"{str(model_key)}-i{int(interval)}-w{int(training_window_months)}-s1of1",
                            "parquet_root": str(Path(args.parquet_root).resolve()),
                            "feature_root": str(Path(feature_root).resolve()),
                            "model_threads": (max(1, int(args.model_threads)) if args.model_threads is not None else None),
                            "telemetry_path": str(run_root),
            ...
            if not accuracy_by_asset:
                raise RuntimeError(
                    f"Neural Stage 2 contract error: no eval metrics generated for interval={int(interval)}m "
                    f"window={int(training_window_months)}m despite sufficient sequence history "
                    f"(train_bars={int(sequence_history['training_window_bars'])}, "
                    f"min_required_history_bars={int(sequence_history['min_required_history_bars'])}, "
...
def main_for_model(model_key: str) -> None:
    ...
    print(f"[{utc_now_iso()}] Neural Stage-2 diagnostic manifest written to {run_root}")
