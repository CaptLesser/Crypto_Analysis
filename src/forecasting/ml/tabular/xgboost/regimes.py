...
try:
    ...
    XGBClassifier = None  # pragma: no cover
...
LOG_FILE = LOG_DIR / "xgboost_regimes.log"

PARQUET_ROOT = Path(
    os.getenv("PIPELINE_PARQUET_XGB_REGIMES_ROOT")
    or resolve_path("output_parquet_root", profile=PIPELINE_PROFILE, required=False)
    or os.getenv("PIPELINE_PARQUET_ROOT", "parquet")
)
OUTPUT_ROOT = PARQUET_ROOT / "XGBoost_Regimes"
...
WINDOW_REEVAL_DAYS = int(os.getenv("XGB_REGIME_WINDOW_REEVAL_DAYS", "7"))
WINDOW_SWITCH_EPS = float(os.getenv("XGB_REGIME_WINDOW_SWITCH_EPS", "0.005"))
...
REFIT_CHECK_M = int(os.getenv("XGB_REGIME_REFIT_M", "3"))
REFIT_MIN_DAILY_SAMPLES = int(os.getenv("XGB_REGIME_REFIT_MIN_DAILY_SAMPLES", "64"))
REFIT_LOGLOSS_MARGIN = float(os.getenv("XGB_REGIME_REFIT_LOGLOSS_MARGIN", "0.02"))
REFIT_ACC_MARGIN = float(os.getenv("XGB_REGIME_REFIT_ACC_MARGIN", "0.01"))
...
XGB_PARAMS = {
    ...
    "num_class": 3,
    "n_estimators": int(os.getenv("XGB_REGIME_N_ESTIMATORS", "200")),
    "max_depth": int(os.getenv("XGB_REGIME_MAX_DEPTH", "4")),
    "learning_rate": float(os.getenv("XGB_REGIME_LEARNING_RATE", "0.05")),
    "subsample": float(os.getenv("XGB_REGIME_SUBSAMPLE", "0.9")),
    "colsample_bytree": float(os.getenv("XGB_REGIME_COLSAMPLE_BYTREE", "0.9")),
    "reg_lambda": float(os.getenv("XGB_REGIME_REG_LAMBDA", "1.0")),
    "random_state": int(os.getenv("XGB_REGIME_RANDOM_STATE", "17")),
    "n_jobs": int(os.getenv("XGB_REGIME_N_JOBS", str(get_model_threads("xgboost_numerics", 8)))),
    "tree_method": os.getenv("XGB_REGIME_TREE_METHOD", "hist"),
}
...
_LOGGER = get_module_logger("xgboost_regimes", LOG_FILE, base_log_fn=lambda m: base_log(f"[xgboost_regimes] {m}"))
...
def _output_file_path(interval: int, year: int, month: int, run_id: str, horizon_minutes: int) -> Path:
    return (
        ...
        / f"year={year}"
        / f"month={month:02d}"
        / f"part-xgboost_regimes_{interval}-{year}{month:02d}-h{horizon_minutes}m-{run_id}.parquet"
    )
...
def _write_month_parts(
    ...
    out_cols = [
        ...
        f"xgb_prob_regime3_high_{horizon_minutes}m",
    ]
    parts: List[dict] = []
...
def main() -> None:
    ...
    try:
        ...
        raise SystemExit(f"[xgb][error] {exc}")
    if not tasks:
        log("[xgb] no tasks to run")
        return
    try:
        acquire_single_run_lock(STATE_ROOT, "xgboost_regimes")
    ...
    for (interval, horizon_minutes, horizon_bars), assets in sorted(groups.items()):
        ...
        for res in unit_results:
            ...
            _write_state(watermarks, pending, progress)
            log(
                f"[xgb] asset={asset} k={interval} h={horizon_minutes}m({horizon_bars}b) "
