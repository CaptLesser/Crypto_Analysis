...
REGIME_RUNNER_SPEC = RegimeFamilyModuleSpec(
    module_slug="lightgbm_regimes",
    family_name=FAMILY,
    prediction_prefix="lgb",
    log_prefix="[lgb]",
    parquet_root_env="PIPELINE_PARQUET_LGB_REGIMES_ROOT",
    progress_seconds_env="LGB_REGIME_PROGRESS_SECONDS",
    source_start_env="LGB_REGIME_SOURCE_START_TS",
    source_end_env="LGB_REGIME_SOURCE_END_TS",
    work_start_env="LGB_REGIME_WORK_START_TS",
    forecast_resume_edge_env="LGB_REGIME_FORECAST_RESUME_EDGE_TS",
    default_unit_workers=1,
    default_model_threads=4,
    max_logical_threads=32,
    thread_env_vars=("LGB_REGIME_N_JOBS",),
...
LGB_PARAMS = {
    ...
    "n_jobs": int(os.getenv("LGB_REGIME_N_JOBS", str(REGIME_RUNNER_SPEC.default_model_threads))),
    "verbosity": -1,
}
...
def main() -> None:
    parser = build_regime_arg_parser(
        REGIME_RUNNER_SPEC,
        description="Walk-forward LightGBM regime forecasting from canonical axis labels.",
    )
