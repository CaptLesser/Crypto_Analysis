...
from src.features.numeric_forecast_profiles import SUPPORTED_REFIT_CADENCES, TABULAR_NUMERIC_PAIR_TRAINING_WINDOW_MONTHS, TABULAR_NUMERIC_TASK_HORIZON_MATRIX
...
from src.forecasting.ml.tabular.shared.tabular_numeric_production_defaults import (
    LIGHTGBM_PRODUCTION_COMBO_REGRESSOR_PROFILES,
    LIGHTGBM_PRODUCTION_DEFAULT_COMBO_WINDOWS,
    LIGHTGBM_TUNED_TASK_REGRESSOR_PROFILES,
)

DEFAULT_INTERVALS = sorted({int(interval) for interval, _, _, _ in LIGHTGBM_PRODUCTION_DEFAULT_COMBO_WINDOWS})
DEFAULT_HORIZON_MINUTES = sorted({int(horizon) for _, horizon, _, _ in LIGHTGBM_PRODUCTION_DEFAULT_COMBO_WINDOWS})
PRODUCTION_DEFAULT_COMBO_WINDOWS = [(int(interval), int(horizon), str(task), int(months)) for interval, horizon, task, months in LIGHTGBM_PRODUCTION_DEFAULT_COMBO_WINDOWS]
...
PRODUCTION_DEFAULT_REFIT_POLICY = {(int(interval), int(horizon), str(task)): ("weekly" if int(interval) <= 60 else "biweekly" if int(interval) <= 240 else "monthly") for interval, horizon, task, _ in PRODUCTION_DEFAULT_COMBO_WINDOWS}
NUMERIC_TASKS = ["log_return", "realized_vol", "true_range", "max_drawdown", "max_runup", "range_efficiency"]
TASK_SHORT = {"log_return": "logret", "realized_vol": "rv", "true_range": "tr", "max_drawdown": "mdd", "max_runup": "mru", "range_efficiency": "reff"}
TASK_LABEL = {"log_return": "future_log_return", "realized_vol": "future_realized_vol", "true_range": "future_true_range", "max_drawdown": "future_max_drawdown", "max_runup": "future_max_runup", "range_efficiency": "future_range_efficiency"}
...
BASE_REG_PARAMS = {"objective": "regression", "random_state": int(os.getenv("LGB_NUMERIC_RANDOM_STATE", "17")), "verbosity": -1, "bagging_freq": 1, "feature_pre_filter": False}
LEGACY_TASK_REGRESSOR_PROFILE = {"n_estimators": int(os.getenv("LGB_NUMERIC_N_ESTIMATORS", "220")), "max_depth": int(os.getenv("LGB_NUMERIC_MAX_DEPTH", "4")), "num_leaves": int(os.getenv("LGB_NUMERIC_NUM_LEAVES", "15")), "learning_rate": float(os.getenv("LGB_NUMERIC_LEARNING_RATE", "0.05")), "feature_fraction": float(os.getenv("LGB_NUMERIC_FEATURE_FRACTION", "0.9")), "bagging_fraction": float(os.getenv("LGB_NUMERIC_BAGGING_FRACTION", "0.9")), "reg_lambda": float(os.getenv("LGB_NUMERIC_REG_LAMBDA", "1.0")), "reg_alpha": float(os.getenv("LGB_NUMERIC_REG_ALPHA", "0.0")), "min_child_samples": int(os.getenv("LGB_NUMERIC_MIN_CHILD_SAMPLES", "20")), "min_split_gain": float(os.getenv("LGB_NUMERIC_MIN_SPLIT_GAIN", "0.0"))}
TUNED_TASK_REGRESSOR_PROFILES: Dict[str, Dict[str, Any]] = dict(LIGHTGBM_TUNED_TASK_REGRESSOR_PROFILES)
COMBO_TUNED_REGRESSOR_PROFILES: Dict[Tuple[int, int, str, int], Dict[str, Any]] = {(int(interval), int(horizon), str(task), int(months)): dict(params) for (interval, horizon, task, months), params in LIGHTGBM_PRODUCTION_COMBO_REGRESSOR_PROFILES.items()}
REG_PARAMS = {**BASE_REG_PARAMS, **LEGACY_TASK_REGRESSOR_PROFILE, "n_jobs": int(os.getenv("LGB_NUMERIC_N_JOBS", str(get_model_threads("lightgbm_numerics", 6))))}
...
def resolve_lgbm_default_combo_profile() -> List[Tuple[int, int, str, int]]:
    return list(PRODUCTION_DEFAULT_COMBO_WINDOWS)


def resolve_lgbm_default_refit_policy(combo: Tuple[int, int, str]) -> Optional[str]:
...
def training_window_override_bars() -> Optional[int]:
    raw = os.getenv("LGB_NUMERIC_TRAIN_WINDOWS", "").strip()
    if not raw:
        return None
    vals: List[int] = []
