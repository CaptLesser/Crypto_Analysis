...
def run_logged_subprocess(command: Sequence[str], *, cwd: Path, env: Dict[str, str], log_path: Path) -> None:
    ...
    with log_path.open("a", encoding="utf-8") as logf:
        ...
        if completed.returncode != 0:
            error = RuntimeError(f"Command failed with exit code {completed.returncode}: {' '.join(command)}")
...
def run_stage0(args: argparse.Namespace, run_root: Path, env: Dict[str, str]) -> None:
    ...
    if stage0_complete(run_root):
        ...
        emit_event(
            ...
            module_name=TABULAR_STAGE0_ENTRYPOINT,
            phase_name="artifact_handoff",
            status="skipped",
            reason_code="stage_already_complete",
            output_path=str(stage0_profile_path(run_root)),
            artifact_profile_source=str(stage0_profile_path(run_root)),
        )
        return
...
def run_stage1(spec: TabularNumericTestModuleSpec, args: argparse.Namespace, run_root: Path, env: Dict[str, str]) -> None:
    ...
    emit_event(run_root, run_id=run_root.name, family="Tabular_Numeric", model=spec.model_key, stage="stage1", function_name="run_stage1", module_name=spec.stage1_module, phase_name="artifact_handoff", status="completed", output_path=str(stage1_selection_path(paths)), artifact_profile_source=str(stage1_selection_path(paths)))
...
def run_stage2(spec: TabularNumericTestModuleSpec, args: argparse.Namespace, run_root: Path, env: Dict[str, str]) -> Path:
    ...
    if manifest_path is None or survivor_path is None:
        emit_event(run_root, run_id=run_root.name, family="Tabular_Numeric", model=spec.model_key, stage="stage2", function_name="run_stage2", module_name=spec.stage2_module, phase_name="artifact_handoff", status="failed", reason_code="artifact_missing", output_path=str(paths.stage2_root))
        raise RuntimeError(f"Stage 2 did not leave complete staged handoff artifacts for {spec.model_key}")
    emit_event(run_root, run_id=run_root.name, family="Tabular_Numeric", model=spec.model_key, stage="stage2", function_name="run_stage2", module_name=spec.stage2_module, phase_name="artifact_handoff", status="completed", output_path=str(survivor_path), artifact_profile_source=str(manifest_path))
    return manifest_path
...
def run_stage3(spec: TabularNumericTestModuleSpec, args: argparse.Namespace, run_root: Path, env: Dict[str, str], *, stage2_manifest: Path) -> None:
    ...
    if stage3_complete(paths):
        progress_line(f"{progress_name}: already complete, skipping")
        emit_event(run_root, run_id=run_root.name, family="Tabular_Numeric", model=spec.model_key, stage="stage3", function_name="run_stage3", module_name=spec.stage3_module, phase_name="artifact_handoff", status="skipped", reason_code="stage_already_complete", output_path=str(paths.stage3_dir))
        return
    ...
    if not stage3_complete(paths):
        emit_event(run_root, run_id=run_root.name, family="Tabular_Numeric", model=spec.model_key, stage="stage3", function_name="run_stage3", module_name=spec.stage3_module, phase_name="artifact_handoff", status="failed", reason_code="stage_artifact_missing", output_path=str(paths.stage3_dir))
        raise RuntimeError(f"Stage 3 did not leave complete artifacts for {spec.model_key}")
    emit_event(run_root, run_id=run_root.name, family="Tabular_Numeric", model=spec.model_key, stage="stage3", function_name="run_stage3", module_name=spec.stage3_module, phase_name="artifact_handoff", status="completed", output_path=str(paths.stage3_dir))
