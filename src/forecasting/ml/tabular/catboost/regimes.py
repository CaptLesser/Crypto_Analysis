...
CB_PARAMS = {
    ...
    "thread_count": int(os.getenv("CB_REGIME_N_JOBS", str(get_model_threads("catboost_numerics", 8)))),
    "verbose": False,
}
...
def _output_file_path(interval: int, year: int, month: int, run_id: str, horizon_minutes: int) -> Path:
    return (
        ...
        / f"year={year}"
        / f"month={month:02d}"
        / f"part-catboost_regimes_{interval}-{year}{month:02d}-h{horizon_minutes}m-{run_id}.parquet"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Walk-forward CatBoost regime forecasting from regime_3 labels.")
    ...
    parser.add_argument("--unit-workers", type=int, default=get_workers("catboost_numerics", "unit_workers", 1))
    args = parser.parse_args()
    args.unit_workers = max(1, int(args.unit_workers))
    resolved_model_threads = cap_model_threads(
        workers=int(args.unit_workers),
        model_threads=get_model_threads("catboost_numerics", 8),
    ...
    for (interval, horizon_minutes, horizon_bars), assets in sorted(groups.items()):
        ...
        def _compute_one(item: Tuple[str, str, Optional[int], Optional[int], Optional[int], set[int], Optional[int]]) -> Dict[str, Any]:
            ...
            pred_col = f"cb_pred_regime3_{horizon_minutes}m"
            prob_low_col = f"cb_prob_regime3_low_{horizon_minutes}m"
            prob_neu_col = f"cb_prob_regime3_neutral_{horizon_minutes}m"
            prob_hi_col = f"cb_prob_regime3_high_{horizon_minutes}m"
        ...
        for res in unit_results:
            ...
            _write_state(watermarks, pending, progress)
            log(
                f"[cb] asset={asset} k={interval} h={horizon_minutes}m({horizon_bars}b) "
