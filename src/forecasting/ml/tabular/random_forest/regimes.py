...
try:
    ...
    RandomForestClassifier = None  # pragma: no cover
...
LOG_FILE = LOG_DIR / "random_forest_regimes.log"

PARQUET_ROOT = Path(
    os.getenv("PIPELINE_PARQUET_RF_REGIMES_ROOT")
    or resolve_path("output_parquet_root", profile=PIPELINE_PROFILE, required=False)
    or os.getenv("PIPELINE_PARQUET_ROOT", "parquet")
)
OUTPUT_ROOT = PARQUET_ROOT / "RandomForest_Regimes"
...
WINDOW_REEVAL_DAYS = int(os.getenv("RF_REGIME_WINDOW_REEVAL_DAYS", "7"))
WINDOW_SWITCH_EPS = float(os.getenv("RF_REGIME_WINDOW_SWITCH_EPS", "0.005"))
WINDOW_SWITCH_COOLDOWN_DAYS = int(os.getenv("RF_REGIME_WINDOW_COOLDOWN_DAYS", "14"))
NEUTRAL_CLASS = 1
CLASS_LABELS = [0, 1, 2]
...
REFIT_MIN_DAILY_SAMPLES = int(os.getenv("RF_REGIME_REFIT_MIN_DAILY_SAMPLES", "64"))
REFIT_LOGLOSS_MARGIN = float(os.getenv("RF_REGIME_REFIT_LOGLOSS_MARGIN", "0.02"))
REFIT_ACC_MARGIN = float(os.getenv("RF_REGIME_REFIT_ACC_MARGIN", "0.01"))
...
RF_PARAMS = {
    "n_estimators": int(os.getenv("RF_REGIME_N_ESTIMATORS", "200")),
    "max_depth": int(os.getenv("RF_REGIME_MAX_DEPTH", "4")),
    "min_samples_split": int(os.getenv("RF_REGIME_MIN_SAMPLES_SPLIT", "2")),
    "min_samples_leaf": int(os.getenv("RF_REGIME_MIN_SAMPLES_LEAF", "1")),
...
def _walk_forward_predict(
    ...
    for axis in REGIME_AXIS_ORDER:
        ...
        out[target.prediction_id_column("rf", horizon_minutes)] = pred_ids
        out[target.prediction_column("rf", horizon_minutes)] = [
            axis_id_to_label(axis, int(label_id), unknown_label=target.unknown_label) for label_id in pred_ids.tolist()
        ]
        for idx, prob_col in enumerate(target.probability_columns("rf", horizon_minutes)):
            out[prob_col] = pd.to_numeric(axis_df[f"prob_{idx}"], errors="coerce").fillna(0.0).astype(float)
        future = future_axis_targets(df, target, horizon_bars)
        out[target.target_available_column("rf", horizon_minutes)] = future.available.astype(bool)
        out[target.prediction_kind_column("rf", horizon_minutes)] = axis_df["kind"].astype(str)

        if axis == "trend":
            compatibility_trend = axis_df

    if compatibility_trend is not None:
        out[f"rf_pred_regime3_{horizon_minutes}m"] = pd.to_numeric(
            compatibility_trend["pred"], errors="coerce"
        ).fillna(NEUTRAL_CLASS).astype("int64")
        trend_probs = compatibility_trend[["prob_0", "prob_1", "prob_2"]].apply(pd.to_numeric, errors="coerce").fillna(0.0)
        out[f"rf_pred_regime3_prob_{horizon_minutes}m"] = trend_probs.max(axis=1).astype(float)
...
def main() -> None:
    ...
    if not tasks:
        ...
        return
    try:
        acquire_single_run_lock(STATE_ROOT, "random_forest_regimes")
